...
const MainScreen = () => {
  ...
  const { prediction: urlPrediction, description: urlDescription } = useLocalSearchParams();

  // State to hold the prediction and description from the API
  const [prediction, setPrediction] = useState(null);
  const [description, setDescription] = useState(urlDescription || "");

  useEffect(() => {
    // If there's a prediction in the URL, use it directly
    if (urlPrediction) {
      setPrediction(parseInt(urlPrediction));
      return;
    }

    // Otherwise, fetch the prediction from the API
    const fetchPrediction = async () => {
      try {
        // Replace with your API endpoint
        const response = await axios.post('http://your-api-endpoint/predict', {
        });
  ...
  const predictionNum = prediction !== null ? parseInt(prediction) : 0;
